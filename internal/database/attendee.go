...
func (m *AttendeeModel) GetByEventAndAttendee(eventId, userId int) (*Attendee, error) {
	...
	query := "SELECT id, user_id, event_id FROM event_attendance WHERE event_id = $1 AND user_id = $2"
...
func (m *AttendeeModel) Delete(userId, eventId int) error {
	...
	query := "DELETE FROM event_attendance WHERE user_id = $1 AND event_id = $2"
...
func (m *AttendeeModel) GetEventsByAttendee(userId int) ([]*Event, error) {
	...
	query := `
		...
		FROM events e
		JOIN event_attendance a ON e.id = a.event_id
		WHERE a.user_id = $1
	`
